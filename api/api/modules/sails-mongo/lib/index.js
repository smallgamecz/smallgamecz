...
module.exports = {
  ...
  define: function (datastoreName, tableName, phmDef, done) {
    ...
    // > Remove `_id`.
    _.remove(uniqueIndexesToCreate, (val) => {
      return val === "_id";
    });
          ...
          function (err) {
            if (err && !_.isError(err)) {
              err = flaverr(
                { raw: err },
                new Error(
                  "Consistency violation: Expecting Error instance, but instead got: " +
                    util.inspect(err)
                )
              );
              return next(err);
      ...
      try {
        if (err) {
          if (err.errmsg === "ns not found") {
            throw flaverr(
              "E_PHM_NOT_FOUND",
              new Error("No such physical model is currently defined.")
            );
